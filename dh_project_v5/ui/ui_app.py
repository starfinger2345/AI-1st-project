...
class SignLanguageTranslatorApp(QWidget):
    def __init__(self, model, encoder):
        """
        사용자 인터페이스를 초기화 하고 배치
            1. 애플리케이션 창의 제목 설정
            2. 아이콘 로드
            3. 카메라 뷰 / 버튼 / 텍스트 입력창 등
        
        클래스 인스턴스 생성
            1. 제스처 인식 인스턴스
            2. 한글 조합 인스턴스
            3. TTS(Text-to-Speech) 인스턴스
            
        시그널 & 슬롯 설정 (UI 요소와 기능들을 연결)
        """
        super().__init__()
    ...
    def toggle_pause_resume(self):
        """
        '일시정지' 버튼
            영상 처리 스레드(VideoThread)의 실행이 일시정지 / 재개
            버튼 텍스트도 '일시정지' <-> '다시 시작' 변경
        """
    ...
    def finalize_sentence(self):
        """
        사용자가 bottom_input 텍스트 입력창에서 enter 키를 누르거나, 문장 완성 제스쳐를 입력(end)
            => 현재 입력창의 텍스트 -> 로그 박스에 추가 & 입력창 초기화
        """
    ...
    def update_text(self, new_char):
        """
        영상 처리 스레드로 부터 인식된 한글 문자를 전달 받는 슬롯
            >> 문자를 한글 조합기에 추가 >> bottom_input 텍스트를 업데이트 (end 입력 시, 현재까지 조합된 문장을 확정하고 TTS 기능 실행)
        """
        if new_char == 'end':
    ...
    def update_image(self, cv_img):
        """
        영상 처리 스레드로부터 실시간 카메라 프레임(cv_img)를 전달받는 슬롯
            >> 이미지를 PyQt 위젯에 표시할 수 있는 형식으로 변환 >> 카메라 뷰(camera_view)에 업데이트
        """
        qt_img = self.convert_cv_qt(cv_img); self.camera_view.setPixmap(qt_img)

    def convert_cv_qt(self, cv_img):
        """OpenCV 형식의 이미지 >> PyQt와 호환되는 QPixmap 형식으로 변환"""
    ...
    def closeEvent(self, event):
        """애플리케이션 창이 닫힐 때, 모든 보조 창과 영상 처리 스레드를 안전하게 종료 & OpenCV 창을 닫음"""
    ...
    def update_tts_volume(self, volume: int):
        """
        음성 출력 볼륨 조절
            설정 창에서 변경된 TTS 값을 HandTTS 객체에 적용
        """
